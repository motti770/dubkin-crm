...
export default function CalendarPage() {
  ...
  // Map activities to their day of month
  ...
  // Build grid cells (start from Sunday)
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24">
      ...
      <header className="pt-8 px-6 pb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="glass-panel px-3 py-1.5 rounded-full text-xs font-semibold text-primary hover:bg-white/80 transition-colors"
          >
            היום
          </button>
        </div>
      ...
      <section className="px-6 mb-4">
        <div className="glass-panel rounded-3xl shadow-glass p-4">
          {/* Month navigation */}
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={prevMonth}
              className="h-8 w-8 rounded-full glass-panel flex items-center justify-center text-slate-600 hover:bg-white/80 transition-colors"
            >
            ...
            {cells.map((day, idx) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center rounded-2xl aspect-square transition-all
                  ...
                  {hasActivities && (
                    <div className="flex gap-0.5 mt-0.5 justify-center">
                      {(activitiesByDay[day] || []).slice(0, 3).map((act, i) => (
                        <span
                          key={i}
                          className={`h-1 w-1 rounded-full ${isSelected ? 'bg-white/70' : getActivityDotColor(act.type)}`}
                        />
                      ))}
                    </div>
                  )}
