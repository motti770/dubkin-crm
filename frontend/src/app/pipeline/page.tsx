        ...
        <div className="mt-2 flex gap-1 flex-wrap">
          {STAGES.filter(s => s !== deal.stage_display).map(stage => {
            ...
            return (
              <button
                key={stage}
                onClick={e => { e.preventDefault(); e.stopPropagation(); onMoveStage(deal.id, stage); }}
                disabled={isPending}
  ...
  return (
    <div className="glass-panel rounded-2xl min-w-[260px] md:min-w-0 md:flex-1 flex flex-col shadow-glass">
      ...
      <div className="p-3 flex flex-col gap-2 flex-1 min-h-[200px] md:min-h-[300px]">
        {deals.map((deal, i) => (
          <DealCard key={deal.id} deal={deal} onMoveStage={onMoveStage} isPending={isPending} index={i} />
        ))}
        {deals.length === 0 && (
    ...
    <div className="pt-8 px-2 md:px-0 pb-24 md:pb-8 page-enter">
      ...
      {isLoading ? (
        <div className="flex md:grid md:grid-cols-6 gap-4 overflow-x-auto pb-4 no-scrollbar snap-x">
          {STAGES.map(s => (
            <div key={s} className="min-w-[260px] md:min-w-0 h-80 rounded-2xl glass-panel animate-pulse snap-start" />
          ))}
        </div>
      ) : (
        <div className="flex md:grid md:grid-cols-6 gap-4 overflow-x-auto pb-6 no-scrollbar snap-x">
          {STAGES.map(stage => (
            <StageColumn
              key={stage}
              stage={stage}
